...
def send_mail_qq(sender_qq, sender_auth_code, receiver, mail_title, mail_content, *args, **kwargs):
    """
...
def send_mail(sender, sender_auth_code, receiver, mail_title, mail_content, *args, **kwargs):
    # Now that only support qq mail as server
    send_mail_qq(sender, sender_auth_code, receiver, mail_title, mail_content, *args, **kwargs)


def send_mail_wrapper(sender, sender_auth_code, receiver, *args, **kwargs):
    def send_to(mail_title, mail_content, *args, **kwargs):
        send_mail(sender, sender_auth_code, receiver, mail_title, mail_content, *args, **kwargs)
    return send_to
...
def get_history_submits(username, password, *args, **kwargs):
    ...
    """
    login_res = requests.post(login_url,
                         data={'openid': '', 'url': 'query', 'redi': '', 'methV': 'courseTab',
    ...
    add_healthy_app = requests.get(healthy_app_url,
    ...
    formpage_res = requests.get(fwd_link, headers={'Connection': 'keep-alive',
    ...
    selfFormWid = re.search("_selfFormWid = '([a-zA-Z0-9]+)'", formpage_res.text).group(1)
    form_history_submit = json.loads(
        re.search('fillDetail = (.*);$',
                  [l for l in formpage_res.text.split('\n') if 'fillDetail = ' in l][0]
                  ).group(1))
    logger.info('Load form page successful, wid: {}, forms of last submit: {}'.format(
...
def verify(username, password, send_mail_instance, *args, **kwargs):
    ...
    send_mail_instance and send_mail_instance(mail_title='Successful submit by DHU healthy form tool',
                       mail_content=('Congratulations! Your healthy form has been ' +
    ...
    return len(form_history_submit) >= 1 and last_submit_datetime.date() == datetime.datetime.today().date()
...
def submit(username, password, send_mail_instance, *args, **kwargs):
    ...
    try:
        fwd_link, selfFormWid, form_history_submit = get_history_submits(username, password)
        ...
        submit_res = requests.post(submit_url + '?wid={}&userId={}'.format(selfFormWid, username),
                                   data=data,
                                   headers={'Origin': 'http://fygrtb.dhu.edu.cn',
                                           'Accept-Encoding': 'gzip, deflate',
                                           'Accept-Language': 'en-US,en;q=0.9,zh-CN;q=0.8,zh;q=0.7,ja;q=0.6',
                                           'User-Agent': UA,
        ...
        if try_cnt >= max_try:
            ...
            send_mail_instance and send_mail_instance(mail_title='Submit failed by DHU healthy form tool',
                               mail_content=("It's very sorry to notice you that after {} tries." +
                               "\nYou must submit the healthy form manully!!").format(try_cnt))
            return 0
        else:
            logger.error('Continue try to submit the new form ({}/{}).'.format(try_cnt, max_try))
            send_mail_instance and send_mail_instance(mail_title='Submit failed by DHU healthy form tool',
                               mail_content=("It's very sorry to notice you that after {} tries." +
    ...
    if os.path.isfile(config_file):
        try:
            with open(config_file) as f:
                ...
                check_config(conf)
                for user in conf['users']:
                    logger.info('Push time for user {} is {}:{} every day.'.format(
                        user['username'], user['hour'], user['minute']))
                    if user.get('receiver_mail'):
                        send_mail_instances.append(send_mail_wrapper(sender=conf['sender_qq'],
            ...
            conf = None
            logger.exception('Exception raised when loading config, try to input config from stdin')
        logger.info('Welcome: {}'.format(conf['users'][0]['username']))
    if not conf:
        test = True
        print('User config not found, please enter your student ID and password for campus Wifi in DHU:')
        username = input('Student ID: ')
        password = input('Password: ')
        if input('Would you wanna use qq mail to receive the latest information about this form tool?\n' +
                 'More info refers to https://service.mail.qq.com/cgi-bin/help?subtype=1&id=28&no=1001256\n' +
                 'Your answer: (y/n)') == 'y':
            qq = input('QQ number as sender: ')
            auth_code = input('SMTP authorization code of corresponding QQ mail: ')
            if input('Do you want to use another mail as receiver for latest information? (y/n)') == 'y':
                receiver = input('Receiver mail: ')
            else:
                receiver = (qq + '@qq.com')
            send_mail_instance = send_mail_wrapper(sender=qq, sender_auth_code=auth_code, receiver=receiver)
            send_mail_instance(mail_title='Test by DHU healthy form tool',
                      mail_content='Congratulations! This email represents that your mail server is configured successfully!')
            send_mail_instances.append(send_mail_instance)
            logger.info('Testing your QQ mail by sending an email to {}...'.format(qq + '@qq.com'))
        else:
            receiver = None
        ...
        try:
            ...
            logger.exception('Exception raised when input config from stdin, exit...')
            sys.exit(-1)
    if test:
        try:
            for user in conf['users']:
                submit(username=user['username'], password=user['password'], send_mail_instance=send_mail_instances[0])
        except Exception:
